...
public class Group7 {
	...
	public static void main(String[] args) throws InterruptedException, FileNotFoundException {
		...
		*/
		...
		//here better for us to run a diff() function to clarify if the output file is correct or not.
	...
	private static String[] sort(String[] toSort) {
		...
		//transform string array into Num array
		Num[] numArray = new Num[toSort.length];
		for (int i=0; i< toSort.length;i++){
			Num newNum = new Num();
			newNum.setValue(Long.parseLong(toSort[i]));
			numArray[i] = newNum;
		}
		
		//sort 
		ourSort.primeQSort(numArray, 0, numArray.length-1);
		
	
		int startPointer = 0;
		int endPointer = 0;
		for (int i = 0; i < numArray.length;i++){
			if (numArray[i].getPrime() != numArray[startPointer].getPrime()){
				endPointer = i-1;
				ourSort.qInsort(numArray, startPointer, endPointer);
				startPointer = endPointer+1;
			}
			
			if (i == numArray.length-1 && numArray[i].getPrime() == numArray[startPointer].getPrime()){
				endPointer = i;
				ourSort.qInsort(numArray, startPointer, endPointer);
			}
		...
		//transform Num array into string array
		for (int i=0; i< numArray.length;i++){
			toSort[i] = Long.toString(numArray[i].getValue());
		}
		return toSort;
